...
**Goal:** Platform **system settings** in SQL for non-secret operational limits; secrets remain env / appsettings / user-secrets.
...
| **S4** | All **EmailIntake** operational knobs + **Pipeline** sync gates + related non-secret intake defaults (`DefaultDomain`, S3 bucket/prefix/region names if non-secret, **MIME retention days**) → settings table. Secrets stay env. |
