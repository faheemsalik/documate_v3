...
namespace Innovoice.Services {
	public interface ITextractTextDetectionService
	{
		...
		Task<DetectDocumentTextResponse> DetectTextLocal(string localPath);
		void WaitForJobCompletion(string jobId, int delay = 5000);
		bool IsJobComplete(string jobId);
		List<GetDocumentTextDetectionResponse> GetJobResults(string jobId);
		Task<DetectDocumentTextResponse> DetectTextS3(string bucketName, string key);
	}
	public class TextractTextDetectionService : ITextractTextDetectionService
	{
		private IAmazonTextract textract;
		private readonly ILogger<ITextractTextDetectionService> logger;
		public TextractTextDetectionService(IAmazonTextract textract, ILogger<ITextractTextDetectionService> logger) {
