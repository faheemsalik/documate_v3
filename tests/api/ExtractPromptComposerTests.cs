...
public class ExtractPromptComposerTests
{
    ...
    public void Empty_system_prompt_falls_back_to_default()
    {
        ...
        Assert.DoesNotContain("Additional document instructions:", result.UserMessage);
        Assert.DoesNotContain("Additional post-process instructions:", result.UserMessage);
    }

    [Fact]
    public void Omits_additional_document_section_when_empty()
    {
    ...
    [Fact]
    public void Includes_additional_document_instructions_and_schema_description()
    {
        ...
        Assert.Contains("Additional document instructions:", result.UserMessage);
    ...
    {
        var result = _composer.Compose("SYS", "inst", "{}", "extra", null);
        Assert.Contains(ExtractPromptDefaults.DocumentTextPlaceholder, result.UserMessage);
    }

    [Fact]
    public void Suggest_system_prompt_includes_instructions_schema_and_additional()
    {
        const string schema =
        ...
        Assert.Contains("additional document instructions", draft, StringComparison.OrdinalIgnoreCase);
    }
}
